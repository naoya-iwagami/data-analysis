...
def is_assistant_generated_png(filename):  
    fname = filename.lower()  
    # 32～64桁の16進数 or UUID形式 + .png  
    return (  
        re.fullmatch(r"[a-f0-9]{32,64}\.png", fname) is not None or  
        re.fullmatch(r"[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\.png", fname) is not None  
    )  
...
with st.sidebar:  
    ...
    # ここでアシスタント生成画像ファイルを除外  
    user_files = [f for f in files if not is_assistant_generated_png(f.filename)]  
